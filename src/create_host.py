...
def add_host(service_instance, cluster, name, ssl_thumb, user, pwd):
	host = get_obj(service_instance.RetrieveContent(),[vim.HostSystem], name)

	if host is not None:
		print("Host %s already exists. Using existing host." % host)
		return host
	else:
		cls = get_obj(service_instance.RetrieveContent(),[vim.ClusterComputeResource], cluster)
		if cls is None:
...
def main():
	...
	host = add_host(service_instance,args.cluster,args.host,ssl_thumbprint,args.user,args.password)
